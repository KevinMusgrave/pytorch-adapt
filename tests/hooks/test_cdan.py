...
from pytorch_adapt.layers import EntropyWeights, MCCLoss, RandomizedDotProduct
...
class TestCDAN(unittest.TestCase):
    ...
    def test_cdan_hook(self):
        ...
        for post_g in [None, BSPHook(), BNMHook(), MCCHook(), AFNHook()]:
            ...
            d_reducer = EntropyReducer(
                apply_to=["d_src_domain_loss", "d_target_domain_loss"],
                default_reducer=MeanReducer(),
            )
            g_reducer = EntropyReducer(
                apply_to=["g_src_domain_loss", "g_target_domain_loss"],
                default_reducer=MeanReducer(),
            )
            data = {
                "src_imgs": src_imgs,
            ...
            with torch.no_grad():
                src_entropy_weights = EntropyWeights()(c_logits[:bs])
                target_entropy_weights = EntropyWeights()(c_logits[bs:])
