    ...
    def test_ist_validator(self):
        ...
        for batch_size in [33, 257, 999, 1000, 1001]:
            for dataset_size in [10, 100, 1000, 2000]:
                validator = ISTValidator(batch_size=batch_size)
                src_features = torch.randn(dataset_size, embedding_size).to(TEST_DEVICE)
                target_features = (
                    torch.randn(dataset_size, embedding_size).to(TEST_DEVICE) + 0.1
                )
