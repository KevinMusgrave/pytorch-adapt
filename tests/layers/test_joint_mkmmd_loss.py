...
class TestJointMKMMDLoss(unittest.TestCase):
    def test_joint_mkmmd_loss_quadratic(self):
        ...
        for dtype in TEST_DTYPES:
            ...
            xx_loss = torch.mean((xx_kernels / len(scales)) ** len(x))
            yy_loss = xx_loss
            xy_loss = torch.mean((xy_kernels / len(scales)) ** len(y))
            correct_loss = xx_loss + yy_loss - 2 * xy_loss

            self.assertTrue(torch.isclose(loss, correct_loss, rtol=1e-2))

    def test_joint_mkmmd_loss_linear(self):
        kernel_scales = l_u.get_kernel_scales(low=-2, high=2, num_kernels=5, base=2)
        for dtype in TEST_DTYPES:
            ...
            correct_loss_list = []
            for idx_base in range(len(x[0]) // 2):
                idx1 = idx_base * 2
                idx2 = idx1 + 1
                xx_kernels = 0
                yy_kernels = 0
                ...
                for s in scales:
                    ...
                    xx_kernels += torch.exp(xx_dists[idx1, idx2] * scale)
                    yy_kernels += torch.exp(yy_dists[idx1, idx2] * scale)
                    xy_kernels1 += torch.exp(xy_dists[idx1, idx2] * scale)
                    xy_kernels2 += torch.exp(xy_dists[idx2, idx1] * scale)

                correct_loss_list.extend(
                    [
                        a / len(scales)
                        for a in [xx_kernels, yy_kernels, xy_kernels1, xy_kernels2]
                    ]
                )

            correct_loss_list = [a ** len(x) for a in correct_loss_list]
            correct_loss = 0
            for i in range(len(correct_loss_list)):
                curr_loss = correct_loss_list[i]
                if i % 4 in [0, 1]:
                    correct_loss += curr_loss
                else:
                    correct_loss -= curr_loss

            correct_loss /= float(len(x[0]) // 2)
